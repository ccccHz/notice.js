...
export class Components {
  ...
  createProgressBar() {
    ...
    if (
        ...
        if (width <= 0) {
          ...
          if (options.animation !== null && options.animation.close !== null) {
            ...
            item.className = item.className.replace(
              new RegExp("(?:^|\\s)" + options.animation.open + "(?:\\s|$)"),
              " "
            );
      ...
      startProgress();
      let mainContiner = document.querySelector(".noticejs");
      mainContiner.addEventListener("mouseenter", (event) => {
        // console.log(event.fromElement);
        stopProgress();
      });
      mainContiner.addEventListener("mouseleave", (event) => {
        // console.log(event.fromElement);
        startProgress();
      });
    }
